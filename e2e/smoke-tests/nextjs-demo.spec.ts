...
test.describe('NextJS Demo - Build Verification', () => {
  ...
  test('should render all example sections without errors', async ({ page }) => {
    const errors: string[] = [];
    page.on('console', msg => {
      if (msg.type() === 'error') {
        errors.push(msg.text());
  ...
  test('should render Image components from @snapkit-studio/nextjs', async ({ page }) => {
    const errors: string[] = [];
    page.on('console', msg => {
      if (msg.type() === 'error') {
        errors.push(msg.text());
  ...
  test('should handle client-side navigation without errors', async ({ page }) => {
    const errors: string[] = [];
    page.on('console', msg => {
      if (msg.type() === 'error') {
        errors.push(msg.text());
  ...
  test('should verify production bundle optimization', async ({ page }) => {
    ...
    const isNextJsApp = await page.evaluate(() => {
      return document.querySelector('script[id="__NEXT_DATA__"]') !== null ||
             window.location.pathname !== undefined;
    });
    expect(isNextJsApp).toBe(true);
  });

  test('should load images with Next.js optimization parameters', async ({ page }) => {
    const errors: string[] = [];
    page.on('console', msg => {
      if (msg.type() === 'error') {
        errors.push(msg.text());
